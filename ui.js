...
window.renderWatchfile = async function() {
  ...
  const iMeta={decision_day:{label:'Decision TODAY',color:'var(--red)',bg:'var(--red-dim)',icon:'⚡'},critical:{label:'Tomorrow',color:'var(--red)',bg:'var(--red-dim)',icon:'⚡'},high:{label:'2–3 Days',color:'var(--amber)',bg:'var(--amber-dim)',icon:'▲'},medium:{label:'This Week',color:'var(--accent)',bg:'var(--accent-dim)',icon:'●'},low:{label:'Next 2 Weeks',color:'var(--muted2)',bg:'var(--surface3)',icon:'○'},watching:{label:'Monitoring',color:'var(--muted)',bg:'var(--surface3)',icon:'◌'}};

  const active   = entries.filter(e=>e.status!=='resolved').sort((a,b)=>{const o={decision_day:0,critical:1,high:2,medium:3,low:4,watching:5};return(o[a.intensity]||5)-(o[b.intensity]||5)||a.daysUntil-b.daysUntil;});
  ...
  const cardHtml = e => {
    ...
    return `<div class="wf-card ${e.status==='resolved'?'wf-card-resolved':''}" onclick="openWatchfileDetail('${e.id}')">
      ...
      <div class="wf-metrics-row">
        ...
        <div class="wf-metric-mini"><span class="wf-metric-label">If Rejected</span><span class="wf-metric-val val-red">-${e.expectedMoveDown||'?'}%</span></div>
...
window.openWatchfileDetail = async function(id) {
  ...
  detail.innerHTML=`<div class="slide-up">
    ...
    <div class="wf-detail-hero">
      ...
      <div class="wf-detail-pdufa">PDUFA: <strong style="color:var(--accent)">${e.pdufaDate}</strong> ${e.status!=='resolved'?'('+e.daysUntil+(e.daysUntil===1?' day':' days')+' away)':''}</div>
    </div>
    <div class="wf-detail-grid">
      ...
      <div class="wf-ds"><div class="wf-ds-label">On Rejection</div><div class="wf-ds-val val-red">-${e.expectedMoveDown||'?'}%</div></div>
    ...
    ${e.dilutionRisk?'<div class="dilution-warning">⚠ Dilution Risk — estimated less than 4 months cash runway. Company may raise capital before or after decision.</div>':''}
    ...
    ${e.status!=='resolved'?`<div class="section"><div class="section-title">Record Decision Outcome</div>
      <div style="display:flex;gap:8px;flex-wrap:wrap">
        ...
        <button class="btn-outcome btn-delayed"  onclick="recordOutcomeUI('${e.id}','delayed')">⏸ Delayed / Withdrawn</button>
      </div>
    ...
    <div class="section"><div class="section-title">Price Chart (1 Month)</div>
      <div class="chart-wrap"><div class="chart-canvas-wrap" id="wfChartWrap"><div class="chart-loading" id="wfChartLoading">⟳ Loading...</div><canvas id="wfChartCanvas" class="chart-canvas" style="display:none"></canvas></div></div>
    </div>
  </div>`;
...
window.recordOutcomeUI = async function(id,outcome) {
  if(!confirm(`Record outcome as "${outcome}"?`))return;
...
function drawConfidenceChart(ctx,data,w,h) {
  ...
  [0,25,50,75,100].forEach(v=>{if(v<mn||v>mx)return;const y=p.t+((mx-v)/range)*ph;ctx.strokeStyle='rgba(255,255,255,.05)';ctx.lineWidth=1;ctx.beginPath();ctx.moveTo(p.l,y);ctx.lineTo(p.l+pw,y);ctx.stroke();ctx.fillStyle='rgba(200,210,230,.3)';ctx.font='8px IBM Plex Mono';ctx.fillText(v+'%',0,y+3);});
  const last=scores[scores.length-1],color=last>=50?getCssVar('green'):getCssVar('red');
  const grad=ctx.createLinearGradient(0,p.t,0,p.t+ph);grad.addColorStop(0,`${color}33`);grad.addColorStop(1,`${color}00`);
  ctx.beginPath();data.forEach((d,i)=>{const x=p.l+(i/(data.length-1))*pw,y=p.t+((mx-d.score)/range)*ph;i===0?ctx.moveTo(x,y):ctx.lineTo(x,y);});ctx.lineTo(p.l+pw,p.t+ph);ctx.lineTo(p.l,p.t+ph);ctx.closePath();ctx.fillStyle=grad;ctx.fill();
  ctx.strokeStyle=color;ctx.lineWidth=1.5;ctx.lineJoin='round';ctx.beginPath();data.forEach((d,i)=>{const x=p.l+(i/(data.length-1))*pw,y=p.t+((mx-d.score)/range)*ph;i===0?ctx.moveTo(x,y):ctx.lineTo(x,y);});ctx.stroke();
...
window.renderSignalsFeed = async function() {
  ...
  const badge=document.getElementById('signalsBadge');if(badge)badge.textContent=signals.length;
...
function renderSignalSidebar(signals) {
  ...
  sb.innerHTML=sorted.map(s=>`
    <div class="ticker-item urg-${s.urgency} ${s.id===window.activeSignalId?'active':''}" data-id="${s.id}" onclick="selectSignal('${s.id}')">
      ...
      <div class="t-type-mini">${s.type==='fda_watchfile'?'FDA':s.type==='squeeze'?'SQZ':s.type==='momentum'?'MOM':s.type==='ma'?'M&A':'CAT'}</div>
    </div>`).join('');
}

window.flashNewSignals=ids=>{ids.forEach(id=>{const el=document.querySelector(`[data-id="${id}"]`);if(el){el.classList.remove('flash');void el.offsetWidth;el.classList.add('flash');}});};
...
window.renderDetail = function(id) {
  dbGet('signals',id).then(sig=>{
    ...
    const typeBanners={
      ...
      momentum:`<div class="type-banner type-momentum"><span>📈</span><div><div class="tb-title">Momentum Continuation</div><div class="tb-sub">Already moving — continuation judged likely</div></div></div>`,
    ...
    const sourcesHtml=(sig.sources||[]).map(s=>`<a class="source-card" href="${s.url}" target="_blank" rel="noopener noreferrer"><div class="source-icon">${s.icon||'📰'}</div><div class="source-body"><div class="source-pub-row"><div class="source-pub">${s.pub}</div><div class="source-time-label">${s.time}</div></div><div class="source-hl">${s.headline}</div><span class="source-url">${s.url}</span></div><div class="source-arrow">→</div></a>`).join('');
    let outcomeHtml;
    ...
    const momSection=sig.type==='momentum'&&(sig.continuationReason||sig.fadeRisks)?`<div class="section"><div class="section-title">Continuation Analysis</div><div class="momentum-analysis">${sig.continuationReason?`<div class="ma-row ma-bull"><div class="ma-icon">🟢</div><div class="ma-body"><div class="ma-label">Why it continues</div><div class="ma-text">${sig.continuationReason}</div></div></div>`:''} ${sig.fadeRisks?`<div class="ma-row ma-bear"><div class="ma-icon">🔴</div><div class="ma-body"><div class="ma-label">Fade risks</div><div class="ma-text">${sig.fadeRisks}</div></div></div>`:''}</div></div>`:'';
    detail.innerHTML=`<div class="slide-up">
      ${banner}
      <div class="signal-hero">
        ...
        <div class="hero-metrics">
          <div class="metric-card"><div class="metric-label">Move</div><div class="metric-val ${moveClass}">${sig.move>=0?'+':''}${sig.move}%</div><div class="metric-sub">${sig.type==='momentum'?'So far':'Current'}</div></div>
      ...
      <div class="section"><div class="section-title">Price Chart</div>
        <div class="chart-wrap"><div class="chart-header"><div class="chart-title">${sig.ticker}</div><div class="chart-toggle"><div class="chart-btn ${window.activeChartMode==='line'?'on':''}" onclick="switchChart('${sig.id}','line',this)">Line</div><div class="chart-btn ${window.activeChartMode==='candle'?'on':''}" onclick="switchChart('${sig.id}','candle',this)">Candle</div></div></div>
          <div class="chart-canvas-wrap" id="chartWrap-${sig.id}"><div class="chart-loading" id="chartLoading-${sig.id}">⟳ Loading chart...</div><canvas id="chartCanvas-${sig.id}" class="chart-canvas" style="display:none"></canvas></div>
        </div>
      </div>
...
window.goToPortfolioAdd=ticker=>{window.activeTab='portfolio';document.querySelectorAll('.tab').forEach(x=>x.classList.remove('active'));document.querySelector('[data-tab="portfolio"]')?.classList.add('active');renderPortfolio(ticker);};

// ============================================================
//  CHARTS
// ============================================================
let _chartLock={};
window.switchChart=(id,mode,btn)=>{window.activeChartMode=mode;document.querySelectorAll('.chart-btn').forEach(b=>b.classList.remove('on'));if(btn)btn.classList.add('on');dbGet('signals',id).then(s=>{if(s)loadSignalChart(id,s.ticker,s.scannedAt);});};

async function loadSignalChart(id,ticker,scannedAt){
  if(_chartLock[id])return;_chartLock[id]=true;
  ...
  try{
    const data=await fetchOHLCV(ticker,'1d','5m');if(!data||!data.length){loading.textContent='Chart unavailable';return;}
    const sigTime=new Date(scannedAt).getTime();const filtered=data.filter(d=>d.t>=sigTime-5*60*1000);const plotData=filtered.length>=3?filtered:data.slice(-40);
    loading.style.display='none';canvas.style.display='block';
    const wrap=document.getElementById(`chartWrap-${id}`),w=(wrap?wrap.offsetWidth:canvas.offsetWidth)||320,h=148;
    canvas.width=w;canvas.height=h;canvas.style.width=w+'px';canvas.style.height=h+'px';
    window.activeChartMode==='candle'?drawCandlestick(canvas.getContext('2d'),plotData,w,h):drawLineChart(canvas.getContext('2d'),plotData,w,h);
  }catch(_){loading.textContent='Chart unavailable';}finally{_chartLock[id]=false;}
}
...
function drawLineChart(ctx,data,w,h){
  ...
  ctx.clearRect(0,0,w,h);
  for(let i=0;i<=4;i++){const y=p.t+(ph/4)*i;ctx.strokeStyle='rgba(255,255,255,.05)';ctx.lineWidth=1;ctx.beginPath();ctx.moveTo(p.l,y);ctx.lineTo(p.l+pw,y);ctx.stroke();ctx.fillStyle='rgba(200,210,230,.28)';ctx.font='8px IBM Plex Mono';ctx.fillText('$'+(maxP-(range/4)*i).toFixed(2),0,y+3);}
  const grad=ctx.createLinearGradient(0,p.t,0,p.t+ph);grad.addColorStop(0,`${color}33`);grad.addColorStop(1,`${color}00`);
  ctx.beginPath();data.forEach((d,i)=>{const x=p.l+(i/(data.length-1))*pw,y=p.t+((maxP-d.c)/range)*ph;i===0?ctx.moveTo(x,y):ctx.lineTo(x,y);});ctx.lineTo(p.l+pw,p.t+ph);ctx.lineTo(p.l,p.t+ph);ctx.closePath();ctx.fillStyle=grad;ctx.fill();
  ctx.strokeStyle=color;ctx.lineWidth=1.5;ctx.lineJoin='round';ctx.beginPath();data.forEach((d,i)=>{const x=p.l+(i/(data.length-1))*pw,y=p.t+((maxP-d.c)/range)*ph;i===0?ctx.moveTo(x,y):ctx.lineTo(x,y);});ctx.stroke();
  ctx.fillStyle='rgba(200,210,230,.28)';ctx.font='8px IBM Plex Mono';[0,Math.floor(data.length/2),data.length-1].forEach(i=>{if(!data[i])return;const x=p.l+(i/(data.length-1))*pw;ctx.fillText(new Date(data[i].t).toLocaleTimeString('en-US',{hour:'2-digit',minute:'2-digit',hour12:true,timeZone:'America/New_York'}),x-14,h-4);});
}
...
function drawCandlestick(ctx,data,w,h){
  ...
  data.forEach((d,i)=>{const x=p.l+(i/data.length)*pw+cw/2,yH=p.t+((maxP-d.h)/range)*ph,yL=p.t+((maxP-d.l)/range)*ph,yO=p.t+((maxP-d.o)/range)*ph,yC=p.t+((maxP-d.c)/range)*ph,up=d.c>=d.o,col=up?green:red;ctx.strokeStyle=col;ctx.lineWidth=1;ctx.beginPath();ctx.moveTo(x,yH);ctx.lineTo(x,yL);ctx.stroke();ctx.fillStyle=up?`${col}88`:col;const bt=Math.min(yO,yC),bh=Math.max(1,Math.abs(yC-yO));ctx.fillRect(x-cw/2,bt,cw,bh);ctx.strokeRect(x-cw/2,bt,cw,bh);});
}
...
async function renderHistory(){
  const detail=getDetail();
  detail.innerHTML=`<div class="history-toolbar"><div style="display:flex;gap:5px">${['24h','7d','all'].map(r=>`<div class="range-btn ${window.historyRange===r?'on':''}" onclick="setHistoryRange('${r}')">${r}</div>`).join('')}</div><div style="display:flex;gap:5px;margin-left:auto">${[['time','Time'],['move','Move'],['conf','Conf']].map(([k,v])=>`<div class="sort-btn ${window.historySortBy===k?'on':''}" onclick="setHistorySort('${k}')">${v}</div>`).join('')}</div><button class="export-btn" onclick="exportCSV()">↓ CSV</button></div><div id="histList">${skeletonHTML(4)}</div>`;
...
window.openHistoryDetail=id=>{window.activeSignalId=id;window.renderDetail(id);const btn=document.createElement('button');btn.className='back-btn';btn.textContent='← History';btn.onclick=renderHistory;const d=getDetail();if(d?.firstChild)d.insertBefore(btn,d.firstChild);};
...
async function renderPortfolio(prefillTicker=''){
  ...
  const enriched=p.positions.map((pos,i)=>{const cur=prices[i].status==='fulfilled'?prices[i].value:null;const curVal=cur?pos.shares*cur:pos.shares*pos.buyPrice;totalCur+=curVal;totalCost+=pos.shares*pos.buyPrice;const gl=cur?parseFloat((curVal-pos.shares*pos.buyPrice).toFixed(2)):null;return{...pos,currentPrice:cur,gl,glPct:gl!==null?(gl/(pos.shares*pos.buyPrice)*100).toFixed(2):null};});
  ...
  getDetail().innerHTML=`<div class="portfolio-wrap slide-up">
    <div class="portfolio-total"><div class="portfolio-total-label">Portfolio Value</div><div class="portfolio-total-val">$${totalCur.toFixed(2)}</div>${totalGL!==0?`<div style="font-size:13px;font-weight:600;color:${glColor};margin-top:3px">${totalGL>=0?'+':''}$${Math.abs(totalGL).toFixed(2)} (${totalGL>=0?'+':''}${(totalGL/Math.max(totalCost,.01)*100).toFixed(2)}%)</div>`:''}</div>
    <div class="portfolio-add"><div class="settings-field-label">Add Position</div><div class="portfolio-add-row"><input class="portfolio-input" id="posTickerInput" placeholder="Ticker" maxlength="5" oninput="this.value=this.value.toUpperCase()" value="${prefillTicker}"/><input class="portfolio-input" id="posSharesInput" placeholder="Shares" type="number" min="0" step="any"/><input class="portfolio-input" id="posPriceInput" placeholder="Buy $" type="number" min="0" step="any"/><button class="btn-accent" onclick="addPositionFromUI()">+ Add</button></div></div>
    ${!p.positions.length?'<div class="empty-state" style="height:20vh"><div class="empty-icon">💼</div><div class="empty-title">No positions yet</div></div>':`<div class="section-title" style="margin-bottom:9px">Positions</div>${enriched.map(pos=>{const glC=pos.gl!=null?(pos.gl>=0?'var(--green)':'var(--red)'):'var(--muted)';return`<div class="position-row"><div class="pos-ticker">${pos.ticker}</div><div class="pos-shares">${pos.shares}sh</div><div class="pos-cost">$${pos.buyPrice}→${pos.currentPrice?'$'+pos.currentPrice.toFixed(2):'—'}</div><div class="pos-gl" style="color:${glC}">${pos.gl!=null?`${pos.gl>=0?'+':''}$${Math.abs(pos.gl).toFixed(0)}<br><span style="font-size:9px">${pos.gl>=0?'+':''}${pos.glPct}%</span>`:'—'}</div><button class="pos-remove" onclick="removePositionFromUI('${pos.ticker}')">✕</button></div>`;}).join('')}`}
    <div style="margin-top:14px"><div class="settings-field-label">Cash balance</div><div style="display:flex;gap:7px;margin-top:5px"><input class="portfolio-input" id="cashInput" placeholder="$0.00" type="number" min="0" step="any" value="${p.cashBalance||''}"/><button class="btn-accent" onclick="saveCashFromUI()">Save</button></div></div>
  </div>`;
  if(prefillTicker)fetchStockPrice(prefillTicker).then(pr=>{const el=document.getElementById('posPriceInput');if(pr&&el)el.value=pr;});
}
window.addPositionFromUI=()=>{const t=document.getElementById('posTickerInput')?.value.trim().toUpperCase(),s=parseFloat(document.getElementById('posSharesInput')?.value),pr=parseFloat(document.getElementById('posPriceInput')?.value);if(!t||!s||!pr||s<=0||pr<=0){showToast('Fill in ticker, shares and price','warn');return;}addPosition(t,s,pr);haptic('light');renderPortfolio();};
window.removePositionFromUI=ticker=>{if(confirm(`Remove ${ticker}?`)){removePosition(ticker);haptic('light');renderPortfolio();}};
...
window.renderAnalytics=async function(){
  ...
  if(!data){html+='<div class="no-data">No outcomes recorded yet.<br>Outcomes are checked 1 hour after each signal fires.</div>';}
  else{
    html+=`<div class="stat-grid"><div class="stat-card"><div class="stat-card-label">Hit Rate</div><div class="stat-card-val val-green" id="analyticsHR">—</div><div class="stat-card-sub">${data.total} checked</div></div><div class="stat-card"><div class="stat-card-label">Hits</div><div class="stat-card-val val-green">${data.hits}</div></div><div class="stat-card"><div class="stat-card-label">Misses</div><div class="stat-card-val val-red">${data.misses}</div></div><div class="stat-card"><div class="stat-card-label">Pending</div><div class="stat-card-val val-accent">${data.pending}</div></div></div>`;
    const typeColors={catalyst:'var(--accent)',fda_watchfile:'var(--purple)',squeeze:'var(--red)',ma:'var(--amber)',momentum:'var(--green)'};
    if(Object.keys(data.byType||{}).length){html+='<div class="analytics-section"><div class="analytics-title">By Signal Type</div>';for(const[type,d]of Object.entries(data.byType))html+=`<div class="bar-row"><div class="bar-label">${type}</div><div class="bar-track"><div class="bar-fill" style="width:${d.pct}%;background:${typeColors[type]||'var(--accent)'}"></div></div><div class="bar-pct" style="color:${typeColors[type]||'var(--accent)'}">${d.pct}%</div><div class="bar-count">${d.total}</div></div>`;html+='</div>';}
    if(Object.keys(data.byUrgency||{}).length){html+='<div class="analytics-section"><div class="analytics-title">By Confidence Tier</div>';const uc={critical:'var(--red)',high:'var(--amber)',medium:'var(--green)'};for(const[tier,d]of Object.entries(data.byUrgency)){if(!d)continue;html+=`<div class="bar-row"><div class="bar-label">${tier}</div><div class="bar-track"><div class="bar-fill" style="width:${d.pct}%;background:${uc[tier]}"></div></div><div class="bar-pct" style="color:${uc[tier]}">${d.pct}%</div><div class="bar-count">${d.total}</div></div>`;}html+='</div>';}
    const L=window._fdaLearningCache;
    if(L&&L.totalResolved>=3){html+='<div class="analytics-section"><div class="analytics-title">FDA Learning Patterns</div>';for(const[ind,d]of Object.entries(L.byIndication)){if(d.total<2)continue;const pct=Math.round(d.correct/d.total*100);const color=pct>=60?'var(--green)':pct>=40?'var(--amber)':'var(--red)';html+=`<div class="bar-row"><div class="bar-label">${ind}</div><div class="bar-track"><div class="bar-fill" style="width:${pct}%;background:${color}"></div></div><div class="bar-pct" style="color:${color}">${pct}%</div><div class="bar-count">${d.total}</div></div>`;}if(L.patternNotes?.length){html+='<div style="margin-top:10px">';L.patternNotes.slice(-5).forEach(p=>html+=`<div class="pattern-note">• ${p.note}</div>`);html+='</div>';}html+='</div>';}
    if(data.best||data.worst){html+='<div class="analytics-section"><div class="analytics-title">Highlights</div><div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">';if(data.best){const mv=((data.best.checkPrice-data.best.basePrice)/data.best.basePrice*100).toFixed(1);html+=`<div class="highlight-card"><div class="highlight-label">🏆 Best</div><div class="highlight-ticker">${data.best.ticker}</div><div class="highlight-detail">+${mv}%<br>${data.best.catalyst}</div></div>`;}if(data.worst){const mv=((data.worst.checkPrice-data.worst.basePrice)/data.worst.basePrice*100).toFixed(1);html+=`<div class="highlight-card"><div class="highlight-label">⚠ Worst</div><div class="highlight-ticker">${data.worst.ticker}</div><div class="highlight-detail">${mv}%<br>${data.worst.catalyst}</div></div>`;}html+='</div></div>';}
  }
  html+='</div>';
...
function renderLearn(){
  const sections=[
    ...
    {icon:'📅',title:'What is a PDUFA Date?',body:`PDUFA stands for Prescription Drug User Fee Act. Under this law, the FDA commits to making a decision on a drug application by a specific deadline — the PDUFA date.\n\nThis makes FDA decisions schedulable in advance. You can know weeks or months ahead of time that a biotech company will get a yes or no on a specific date. This is what makes FDA plays uniquely predictable compared to other catalysts.\n\nVERTEX tracks all upcoming PDUFA dates and monitors each company's stock, options activity, and insider buying as the date approaches. The closer to the date, the more intensively it watches.`},
    {icon:'🔥',title:'What is a Short Squeeze?',body:`Short selling is when investors borrow a stock and sell it, hoping to buy it back cheaper later and pocket the difference. Short interest is what percentage of a company's float is sold short.\n\nA short squeeze happens when a heavily shorted stock gets a positive catalyst. Short sellers rush to cover (buy back) their positions all at once, driving the price up rapidly — which forces more short sellers to cover, driving it up further.\n\nThe ingredients for a squeeze: high short float (>20%), low float size (thin = easier to squeeze), and a positive catalyst. VERTEX screens for all three together.`},
    {icon:'📊',title:'Float Size & Short Float',body:`Float shares is the number of shares available for public trading. A stock with 3 million float shares moves very differently from one with 300 million.\n\nWhy: if you need to buy $10M of a $500M float stock, your buying barely moves the price. If you need to buy $10M of a $3M float stock, you've bought most of the available supply — price rockets.\n\nSmall float + positive catalyst = explosive move potential. VERTEX flags float size on every signal.\n\nShort float (or short interest %) is what fraction of the float is currently sold short. Above 20% is elevated. Above 30% is extreme and indicates a potential squeeze setup.`},
    {icon:'📈',title:'What is Options Flow?',body:`Options are contracts giving the right to buy (calls) or sell (puts) a stock at a specific price before a specific date.\n\nUnusual options activity means someone is buying a large, unusual amount of calls or puts on a stock — often before news breaks. This is frequently a sign that informed money (institutional traders, insiders' networks) expects a significant move.\n\nWhen VERTEX detects unusual call buying on a stock with an upcoming FDA date, it significantly increases the confidence score. Unusual options + PDUFA date upcoming is one of the strongest combined signals.`},
    {icon:'📋',title:'What is Insider Buying?',body:`Corporate insiders (executives, directors) must report their stock purchases and sales to the SEC via Form 4 filings, typically within 2 business days.\n\nInsider buying — especially when multiple executives buy at the same time — is a strong signal that they believe the stock will go up. They know their own company better than anyone.\n\nInsider buying before an FDA decision is particularly significant. It doesn't guarantee approval, but it's meaningful evidence that those closest to the situation are optimistic.\n\nVERTEX monitors OpenInsider and SEC EDGAR for Form 4 filings on every watched stock.`},
    {icon:'📄',title:'What is an 8-K Filing?',body:`An 8-K is a "current report" filed with the SEC when a material event occurs that shareholders need to know about. Material events include:\n\n• FDA approvals or rejections\n• Earnings announcements\n• Mergers and acquisitions\n• Major contracts or partnerships\n• Executive changes\n• Bankruptcy or financial distress\n\nSEC EDGAR publishes 8-K filings in a real-time RSS feed. VERTEX polls this feed on every scan to catch material events the moment they're filed — before most news sites pick them up.`},
    {icon:'🤝',title:'What are M&A Signals?',body:`Mergers and acquisitions (M&A) are when one company buys another, typically at a significant premium to the current stock price. The target company's stock usually jumps 20–50% on the announcement.\n\nWhile M&A is the hardest catalyst to predict, there are often signs of informed accumulation beforehand:\n\n• Unusual call option buying weeks before announcement\n• Insiders buying across multiple executives\n• Activist investors disclosing large stakes (13D/13G filings)\n• Unexplained volume increases with no news\n\nVERTEX flags stocks showing 2 or more of these signals simultaneously.`},
    {icon:'🧠',title:'How VERTEX Learns',body:`Every time a prediction is made and the outcome is recorded, VERTEX updates its learning model. It tracks accuracy by:\n\n• Drug indication (cancer, rare disease, CNS, etc.)\n• Company market cap size\n• Short float level at time of prediction\n• Whether unusual options activity was present\n• Whether insider buying was present\n\nAfter every 5 resolved predictions, Claude reviews the pattern and writes a one-sentence observation. Over time these patterns get injected into every future scan, making predictions progressively more accurate for your specific history.\n\nThis is not machine learning in the traditional sense — it's structured self-reflection where Claude reads its own track record and adjusts accordingly.`},
    {icon:'📖',title:'Glossary',body:`PDUFA Date — FDA decision deadline, known in advance\nShort Float — % of float shares sold short\nFloat Shares — shares available for public trading\nBorrow Rate — cost to short a stock; rising = shorts nervous\nForm 4 — SEC filing for insider stock transactions\n8-K — SEC filing for material corporate events\n13D/13G — SEC filings when investor acquires >5% stake\nNDA — New Drug Application (small molecule drugs)\nBLA — Biologics License Application (antibody drugs)\nCatalyst — an event that triggers a significant stock move\nBinary Event — outcome is one of two possibilities (approve/reject)\nMarket Cap — total market value of all shares outstanding\nVolume Ratio — today's volume vs. average daily volume\nShort Squeeze — rapid price increase forced by short sellers covering\nUnusual Options — options volume significantly above normal\nDilution Risk — company may issue new shares, reducing existing share value`},
  ];
  ...
  getDetail().innerHTML=`<div class="settings-wrap slide-up">
    <div class="settings-section"><div class="settings-label">API Connection</div>
      <div class="settings-card">
        <div class="settings-field"><div class="settings-field-label">Anthropic API Key</div><input class="settings-input" id="apiKeyInput" type="password" placeholder="sk-ant-api03-••••••••" autocomplete="off" value="${isConn?'••••••••••••••••••••':''}"/></div>
        ...
        <div class="settings-meta" style="margin-top:10px">Model: <span>claude-haiku-4-5-20251001</span> · Last scan: <span>${lastScan}</span> · Total: <span>${scanCount}</span></div>
    ...
    <div class="settings-section"><div class="settings-label">FDA Watchfile</div>
      <div class="settings-card">
        ...
        <div class="settings-hint">FDA calendar refreshes automatically once per day. Manual refresh fetches the latest PDUFA dates now.</div>
      </div>
    </div>

    <div class="settings-section"><div class="settings-label">Scan Modules</div>
      <div class="settings-card">
        ${[['fdaEnabled','Catalyst Scanner (FDA-aware)'],['squeezeEnabled','Short Squeeze Setup Scanner'],['maEnabled','M&A Accumulation Scanner'],['momentumEnabled','Momentum Continuation Scanner']].map(([k,label])=>`
    ...
    <div class="settings-section"><div class="settings-label">Signal Filters</div>
      <div class="settings-card">
        ...
        <div class="settings-field"><div class="settings-field-label">Momentum min move: <strong style="color:var(--accent)" id="momVal">${s.momentumMinMove}%</strong></div>
          <input class="settings-slider" type="range" min="3" max="20" step="1" value="${s.momentumMinMove}" oninput="document.getElementById('momVal').textContent=this.value+'%'" onchange="updateSetting('momentumMinMove',parseInt(this.value))"/>
        </div>
      </div>
    </div>
    ...
    <div class="settings-section"><div class="settings-label">Display</div>
      <div class="settings-card">
        <div class="settings-field"><div class="settings-field-label">Theme</div>
          <button class="btn-secondary" id="themeToggleBtn" onclick="toggleTheme();renderSettings()">${getTheme()==='dark'?'☀ Switch to Light Mode':'☾ Switch to Dark Mode'}</button>
        </div>
    ...
    <div class="settings-section"><div class="settings-label">Notifications</div>
      <div class="settings-card">
        ...
        <div class="settings-hint">Notifications for: FDA decision day alerts, critical signals, unusual options on watched stocks. Enable after testing phase is complete.</div>
      </div>
    </div>
...
window.refreshFDACalendarManual=async()=>{
  ...
  try{await fetchFDACalendar();showToast(`FDA calendar updated — ${(window._fdaCalendarCache||[]).length} events`,'success');if(typeof window.renderWatchfile==='function')window.renderWatchfile();}
...
window.confirmClearData=async()=>{if(!confirm('Clear ALL data? This cannot be undone.'))return;await clearAllData();showToast('All data cleared','success');renderSettings();};
window.confirmResetSettings=()=>{if(!confirm('Reset all settings to default?'))return;resetSettings();renderSettings();showToast('Settings reset','success');};
