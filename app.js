...
// ── Runtime State ─────────────────────────────────────────────
let isScanning     = false;   // reset on every page load
...
function isMarketOpen() {
  ...
  const open  = (s.marketOpenHour  || 9)  * 60 + (s.marketOpenMin  || 0);
  const close = (s.marketCloseHour || 20) * 60 + (s.marketCloseMin || 0);
...
function msUntilNextMarketOpen() {
  ...
  const openSec = (s.marketOpenHour || 9) * 3600 + (s.marketOpenMin || 0) * 60;
  const closeSec= (s.marketCloseHour||20) * 3600 + (s.marketCloseMin||0) * 60;
  let daysAhead = 0;
...
// ============================================================
//  TRIGGER SCAN  (manual always works, auto only during market hours)
// ============================================================
async function triggerScan({ isPreload = false, isManual = false } = {}) {
  // ── BUG FIX: never block a manual scan on market hours ────
  if (isScanning) return;
  const apiKey = getApiKey();
  if (!apiKey) { showScanError('Enter your API key in Settings first.'); return; }

  // Auto-scans respect market hours. Manual scans always go through.
...
// ── BUG FIX: always call with isManual:true from button ──────
...
function addToWatchlist(ticker) {
  ...
  if (!t || !/^[A-Z]{1,5}$/.test(t)) return false;
  if (w.find(x => x.ticker === t)) return false;
...
function removeFromWatchlist(ticker) {
  saveWatchlist(loadWatchlist().filter(x => x.ticker !== ticker.toUpperCase()));
}

function isOnWatchlist(ticker) { return !!loadWatchlist().find(x => x.ticker === ticker.toUpperCase()); }

// Fetch watchlist data: price + news via Claude
async function fetchWatchlistData(item) {
  const price = await fetchStockPrice(item.ticker);
  let pctChange = null;
  if (price && item.addedPrice) {
    pctChange = parseFloat(((price - item.addedPrice) / item.addedPrice * 100).toFixed(2));
  }
  return { ...item, currentPrice: price, pctChange };
}
  ...
  try {
    const res = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        messages: [{ role: 'user', content: `Search for the latest news headlines about ${ticker} stock today. Return ONLY a JSON array of up to 4 headlines, no markdown: [{"headline":"...","source":"...","url":"...","time":"..."}]. If nothing found return [].` }]
      })
    });
    if (!res.ok) return [];
...
function loadPortfolio() {
  try { return JSON.parse(localStorage.getItem(PORTFOLIO_KEY)||'{"positions":[],"cashBalance":0,"totalInvested":0}'); }
  catch { return { positions: [], cashBalance: 0, totalInvested: 0 }; }
}
function savePortfolio(p){ try { localStorage.setItem(PORTFOLIO_KEY, JSON.stringify(p)); } catch (_) {} }

function loadPortfolioHistory() {
  try { return JSON.parse(localStorage.getItem(PORTFOLIO_HIST_KEY)||'[]'); } catch { return []; }
}
...
function addPosition(ticker, shares, buyPrice) {
  ...
  if (existing) {
    ...
    const avgPrice = ((existing.shares * existing.buyPrice) + (shares * buyPrice)) / totalShares;
  ...
  p.totalInvested = p.positions.reduce((a,x) => a + x.shares * x.buyPrice, 0);
  savePortfolio(p);
  snapshotPortfolio();
}
...
function updatePortfolioCash(amount) {
  const p = loadPortfolio(); p.cashBalance = parseFloat(amount)||0; savePortfolio(p);
}
...
async function snapshotPortfolio() {
  ...
  let totalValue = p.cashBalance || 0;
  p.positions.forEach((pos, i) => {
    const price = prices[i].status==='fulfilled' ? prices[i].value : null;
    if (price) totalValue += pos.shares * price;
    else totalValue += pos.shares * pos.buyPrice; // fallback to cost
  });
  const hist = loadPortfolioHistory();
  hist.push({ t: Date.now(), v: parseFloat(totalValue.toFixed(2)) });
  // Keep last 90 days of snapshots
  const cutoff = Date.now() - 90 * 24 * 60 * 60 * 1000;
  savePortfolioHistory(hist.filter(h => h.t > cutoff));
}
...
function buildSystemPrompt() {
  const s = loadSettings(); const mc = getMcapPreset();
  ...
  return BASE_SYSTEM_PROMPT
    ...
    + (watchlist.length ? `- PRIORITY WATCHLIST (pay extra attention): ${watchlist.join(', ')}\n` : '');
}
...
function buildUserPrompt() {
  ...
  return `Current time: ${t} ET, ${d}\n\nSearch all 12 sources for stocks in the $${mc.min>=1000?(mc.min/1000)+'B':mc.min+'M'} to $${mc.max>=1000?(mc.max/1000)+'B':mc.max+'M'} market cap range with hard catalysts confirmed in the last 4 hours.\n\nMin confidence: ${s.minConfidence||50}%. Min move: ${s.minMove||5}%.\n\nReturn [] if nothing qualifies.`;
}
...
async function fetchOHLCV(ticker,range='1d',interval='5m'){
  ...
  try{
    ...
    for(let i=0;i<ts.length;i++){
      ...
      result.push({t:ts[i]*1000,o:parseFloat((q.open[i]||0).toFixed(4)),h:parseFloat((q.high[i]||0).toFixed(4)),l:parseFloat((q.low[i]||0).toFixed(4)),c:parseFloat((q.close[i]||0).toFixed(4)),v:q.volume[i]||0});
    }
    return result;
  }catch{return null;}
}
...
async function fetchBasePrice(signal) {
  ...
  if(price!==null){
    ...
    // Also set addedPrice if on watchlist
    const w=loadWatchlist();
    const wi=w.find(x=>x.ticker===signal.ticker);
...
async function checkOutcome(signalId) {
  ...
  const pSign=signal.move>=0?'+':'',aSign=actual>=0?'+':'';
  updateSignalInStorage(signalId,{outcome:isHit?'hit':'miss',outcomeMsg:`${pSign}${signal.move}% signal → ${aSign}${actual}% actual. ${isHit?'HIT ✓':'MISS ✗'} (Base $${signal.basePrice} → Now $${current})`,checkPrice:current});
...
function getSignalsFor24hTab(){
  ...
  if(day===6||day===0){
    const db=day===6?1:2;const fs=new Date(et);fs.setDate(fs.getDate()-db);fs.setHours(0,0,0,0);
...
function calcAccuracy(signals){
  ...
  const avgPredicted=avg(withPred.map(s=>Math.abs(s.move)));
  const avgActual=avg(withPred.map(s=>Math.abs(((s.checkPrice-s.basePrice)/s.basePrice)*100)));

  // Accuracy trend (split into first half vs second half)
  let trend=null;
  if(checked.length>=6){
    const half=Math.floor(checked.length/2);
    const first=checked.slice(half),second=checked.slice(0,half);
    const r1=first.filter(s=>s.outcome==='hit').length/first.length*100;
    const r2=second.filter(s=>s.outcome==='hit').length/second.length*100;
  ...
  return{overall,total:checked.length,hits,misses:checked.length-hits,
    ...
    avgHitMove:avg(hm),avgMissMove:avg(mm),
    avgPredicted,avgActual,best,worst,trend};
}
...
async function buildEmptyStateContent(){
  ...
  const priceRows=past.map((s,i)=>{
    ...
    const ms=current&&s.basePrice?((current-s.basePrice)/s.basePrice*100).toFixed(1):null;
    const sign=ms>=0?'+':'';const color=ms===null?'var(--muted)':ms>=0?'var(--green)':'var(--red)';
    ...
    return`<div class="es-row" onclick="window.selectSignal&&selectSignal('${s.id}')"><div class="es-ticker">${s.ticker}</div><span class="tag ${s.catalystTag} es-catalyst">${s.catalyst}</span><div class="es-date">${dt}</div><div class="es-move" style="color:${color}">${ms!==null?sign+ms+'%':'—'}</div><div class="es-outcome">${outcome}</div></div>`;
  }).join('');
...
// ============================================================
//  INIT  (isScanning reset on every page load)
// ============================================================
function initScanner(){
  isScanning = false; // ── BUG FIX: always reset on load ──
  releaseScanLock();  // ── BUG FIX: release any stale lock ──
  ...
  // Snapshot portfolio value once per session
  setTimeout(snapshotPortfolio, 3000);
